...
Player.prototype.update = function(deltaTime)
{
	...
	if(keyboard.isKeyDown(keyboard.KEY_SPACE) == true && this.cooldownTimer <= 0) {
		...
		if (this.cooldownTimer <= 0) {
			sfxFire.play();
			var bullet = new Bullet (this.position.x, this.position.y, this.moveRight);
			bullets.push(bullet);
		...
		//play the animation
		//check the cooldownTimer
			//shoot sfxFire
			//var byllet = new Bullet (playerx,playery, true )
	...
	if ((wasleft && (this.velocity.x > 0)) ||(wasright && (this.velocity.x < 0)))
	{
		...
		if (this.velocity.y > 0) {
			...
			if(cellAtTileCoord(LAYER_OBJECT_TRIGGERS, tx, ty) == true)
			{
					...
					context.fillText("The End Press F5 to play again!", 200, 240);
